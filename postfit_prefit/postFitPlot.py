...
##ROOT.gStyle.SetLabelFormat("%.1f", "X")
...
legend.AddEntry(h_sig, "m_{Z'}=1GeV; m_{DM}=150GeV; m_{med}=3TeV (prefit exp.)", "L")
...
legend_2.AddEntry(h_sig_2, "m_{Z'}=1GeV; m_{DM}=%sGeV; m_{med}=2TeV (prefit exp.)"%(Mx_sig2), "L")#x10^{5}
...
#print("Number of bins in ratio plot:", ratio_nom.GetNbinsX())
...
c1.SaveAs("postfit_splusb_%s_paper.pdf"%second_dir)
c1.SaveAs("postfit_splusb_%s_paper.png"%second_dir)
